import { ArrowUpDown, Upload, X } from 'lucide-react'
import { useRef, useState } from 'react'
import { rearrangePdfPagesApi } from '../api_caller/rearrange_pdf_pages'
...
export default function RearrangePdfPagesPage() {
	...
	async function handleRearrangePages() {
		...
		try {
			...
			setStatusMessage('Rearranging pages...')

			const blob = await rearrangePdfPagesApi(file, pageOrder)
			const url = window.URL.createObjectURL(new Blob([blob], { type: 'application/pdf' }))
			const link = document.createElement('a')
			const safeName = file.name.toLowerCase().endsWith('.pdf') ? file.name.slice(0, -4) : file.name

			link.href = url
			link.download = `${safeName}-pages-rearranged.pdf`
			document.body.appendChild(link)
			link.click()
			link.remove()
			window.URL.revokeObjectURL(url)

			setStatusMessage(`Downloaded ${safeName}-pages-rearranged.pdf`)
		} catch (error) {
			setStatusMessage(error instanceof Error ? error.message : 'Rearrange pages failed.')
		} finally {
	...
	return (
		<main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(96,165,250,0.16),_transparent_28%),linear-gradient(180deg,_#1f2937_0%,_#111827_52%,_#030712_100%)] px-6 py-10 text-slate-100">
			<div className="mx-auto max-w-7xl">
				<div className="mb-8">
					<div className="inline-flex items-center gap-2 rounded-full border border-blue-400/20 bg-blue-400/10 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.3em] text-blue-200">
				...
				<div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
						...
						<div
							...
							className={`flex min-h-[280px] cursor-pointer items-center justify-center rounded-3xl border-2 border-dashed bg-slate-900/70 p-8 text-center shadow-xl shadow-black/20 ring-1 ring-white/5 transition ${
								isDraggingOver
									...
									: 'border-slate-600 hover:border-blue-400/80'
							}`}
						>
							<div>
						...
						<section className="rounded-3xl border border-slate-700/80 bg-slate-900/75 p-6 shadow-xl shadow-black/20 ring-1 ring-white/5">
							<div className="mb-4 flex items-center justify-between gap-4">
								<div>
									...
									<p className="text-sm text-slate-300">Upload a PDF, then enter the new page order</p>
								</div>
							...
							{!file ? (
								...
								<div className="rounded-2xl border border-slate-700 bg-slate-950/80 p-4 shadow-lg shadow-black/20 ring-1 ring-white/5">
									<div className="flex items-start justify-between gap-3">
										<div>
											<p className="text-xs font-semibold text-blue-300">PDF</p>
					...
					<aside className="rounded-3xl border border-slate-700/80 bg-slate-900/75 p-6 shadow-xl shadow-black/20 ring-1 ring-white/5">
						<h3 className="text-lg font-semibold text-slate-100">Page order</h3>
						...
						<div className="mt-5 space-y-5">
							<label className="block">
								...
								<input
									...
									className="w-full rounded-xl border border-slate-600 bg-slate-950 px-3 py-2 text-sm text-slate-100 outline-none transition focus:border-blue-400"
								/>
								<p className="mt-2 text-xs leading-relaxed text-slate-400">
									Enter the full page order exactly once for every page. Example: `3,1,2,4,5`
								</p>
							</label>
							...
							<div className="rounded-2xl border border-slate-700 bg-slate-950/60 p-4">
								...
								<p className="mt-2 text-sm leading-relaxed text-slate-300">
									The PDF pages will be rearranged to match the order you enter, then the updated file will download automatically.
								</p>
								{statusMessage ? (
							...
							<button
								...
								disabled={!file || !pageOrder.trim() || isRearranging}
								className="w-full rounded-xl bg-blue-400 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-blue-300 disabled:cursor-not-allowed disabled:opacity-50"
							>
								{isRearranging ? 'Rearranging pages...' : 'Rearrange PDF pages'}
							</button>
						</div>
					</aside>
				</div>
