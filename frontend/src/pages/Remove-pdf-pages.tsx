import { FileMinus2, Upload, X } from 'lucide-react'
import { useRef, useState } from 'react'
import { removePdfPagesApi } from '../api_caller/remove_pdf_pages'
...
export default function RemovePdfPagesPage() {
	...
	async function handleRemovePages() {
		...
		try {
			...
			const blob = await removePdfPagesApi(file, pagesToRemove)
			const url = window.URL.createObjectURL(new Blob([blob], { type: 'application/pdf' }))
			const link = document.createElement('a')
			const safeName = file.name.toLowerCase().endsWith('.pdf') ? file.name.slice(0, -4) : file.name

			link.href = url
			link.download = `${safeName}-pages-removed.pdf`
			document.body.appendChild(link)
			link.click()
			link.remove()
			window.URL.revokeObjectURL(url)

			setStatusMessage(`Downloaded ${safeName}-pages-removed.pdf`)
		} catch (error) {
			setStatusMessage(error instanceof Error ? error.message : 'Remove pages failed.')
		} finally {
			setIsRemoving(false)
		}
	}
	...
	return (
		<main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(248,113,113,0.16),_transparent_28%),linear-gradient(180deg,_#1f2937_0%,_#111827_52%,_#030712_100%)] px-6 py-10 text-slate-100">
			<div className="mx-auto max-w-7xl">
				<div className="mb-8">
					<div className="inline-flex items-center gap-2 rounded-full border border-rose-400/20 bg-rose-400/10 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.3em] text-rose-200">
						...
						<section className="rounded-3xl border border-slate-700/80 bg-slate-900/75 p-6 shadow-xl shadow-black/20 ring-1 ring-white/5">
							<div className="mb-4 flex items-center justify-between gap-4">
								<div>
									...
									<p className="text-sm text-slate-300">Upload a PDF, then choose pages to remove</p>
								</div>
							...
							{!file ? (
								...
								<div className="rounded-2xl border border-slate-700 bg-slate-950/80 p-4 shadow-lg shadow-black/20 ring-1 ring-white/5">
									<div className="flex items-start justify-between gap-3">
										<div>
											<p className="text-xs font-semibold text-rose-300">PDF</p>
						...
						<div className="mt-5 space-y-5">
							<label className="block">
								...
								<input
									type="text"
									value={pagesToRemove}
									onChange={(event) => setPagesToRemove(event.target.value)}
									placeholder="Example: 1,3,5-7"
									className="w-full rounded-xl border border-slate-600 bg-slate-950 px-3 py-2 text-sm text-slate-100 outline-none transition focus:border-rose-400"
								/>
							...
							<div className="rounded-2xl border border-slate-700 bg-slate-950/60 p-4">
								...
								<p className="mt-2 text-sm leading-relaxed text-slate-300">
									The selected pages will be removed, then the updated PDF will download automatically.
								</p>
								{statusMessage ? (
							...
							<button
								...
								disabled={!file || !pagesToRemove.trim() || isRemoving}
								className="w-full rounded-xl bg-rose-400 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-rose-300 disabled:cursor-not-allowed disabled:opacity-50"
							>
								{isRemoving ? 'Removing pages...' : 'Remove PDF pages'}
							</button>
						</div>
					</aside>
				</div>
