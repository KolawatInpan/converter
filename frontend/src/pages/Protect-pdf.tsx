import { Lock, Shield, Upload, X } from 'lucide-react'
import { useRef, useState } from 'react'
import { protectPdfApi } from '../api_caller/protect_pdf'
...
export default function ProtectPdfPage() {
	...
	async function handleProtectPdf() {
		...
		try {
			...
			const blob = await protectPdfApi(file, password)
			const url = window.URL.createObjectURL(new Blob([blob], { type: 'application/pdf' }))
			const link = document.createElement('a')
			const safeName = file.name.toLowerCase().endsWith('.pdf') ? file.name.slice(0, -4) : file.name

			link.href = url
			link.download = `${safeName}-protected.pdf`
			document.body.appendChild(link)
			link.click()
			link.remove()
			window.URL.revokeObjectURL(url)

			setStatusMessage(`Downloaded ${safeName}-protected.pdf`)
		} catch (error) {
			setStatusMessage(error instanceof Error ? error.message : 'Protect PDF failed.')
		} finally {
						...
						<section className="rounded-3xl border border-slate-700/80 bg-slate-900/75 p-6 shadow-xl shadow-black/20 ring-1 ring-white/5">
							<div className="mb-4 flex items-center justify-between gap-4">
								<div>
									...
									<p className="text-sm text-slate-300">Upload a PDF, then set a password</p>
								</div>
						...
						<div className="mt-5 space-y-5">
							<label className="block">
								...
								<div className="relative">
									<Lock className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-emerald-300" />
									<input
										type="password"
										value={password}
										onChange={(event) => setPassword(event.target.value)}
										placeholder="Enter a password"
										className="w-full rounded-xl border border-slate-600 bg-slate-950 py-2 pl-10 pr-3 text-sm text-slate-100 outline-none transition focus:border-emerald-400"
									/>
								</div>
							...
							<div className="rounded-2xl border border-slate-700 bg-slate-950/60 p-4">
								...
								<p className="mt-2 text-sm leading-relaxed text-slate-300">
									The uploaded PDF will be encrypted with your password, then the protected file will download automatically.
								</p>
								{statusMessage ? (
							...
							<button
								...
								disabled={!file || !password.trim() || isProtecting}
